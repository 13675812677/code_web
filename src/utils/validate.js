...
export function globalValid(rule, value, callback){
  ...
  value = value || ''
  let tips = Vue.$t('gloabl_validate.max_length1')+limitObj.max+Vue.$t('gloabl_validate.max_length2')
  if(typeof value != 'undefined' && value != null){
    if(value.indexOf(" ") != -1 ){
      callback(new Error(Vue.$t('gloabl_validate.not_space')))
...
export function letterNumberLineValid(rule,value,callback){
  ...
  if (!myreg.test(value)) {
    callback(new Error(Vue.$t('gloabl_validate.uppercase_number_line')))
  } else {
    callback()
  }
}
...
export function UpperCasevalidate(rule,value,callback) {
  const reg = /^[A-Z]+$/
  if(!reg.test(value)){
    callback(new Error(Vue.$t('gloabl_validate.uppercase')))
  }else{
    callback()
  }
}
...
export function UpperCaseLinevalidate(rule,value,callback) {
  const reg = /^[A-Z_]+$/
  if(!reg.test(value)){
    callback(new Error(Vue.$t('gloabl_validate.uppercase_line')))
  }else{
    callback()
  }
}