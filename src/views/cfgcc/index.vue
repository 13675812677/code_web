<!-- The Chaincode Configuration Component -->
<template>
  	<div class="app-container">
		...
		<CFGCCEditor ref="editor" @onOK="saveEntity"></CFGCCEditor>
  	</div>
...
<script>
	...
	import CFGCCEditor from '@/views/cfgcc/editor'
  	export default {
		// 注册依赖组件
		components : {CFGCCEditor},
    	data() {
      	  	return {
				// 定义表格字段
        		header: [
					{title: this.$t('common.name'), field: 'name', class: ''},
					{title: this.$t('cfgcc.channelName'), field: 'channelName', class: ''},
					{title: this.$t('cfgcc.chaincodeName'), field: 'chaincodeName', class: ''}
        		],
				// 翻页数据格式
				pagination : {
					page: 1,
    	...
    	methods: {
			doSearch() {// 执行查询
				getJson('api/config/chaincode/paginate', this.conditions, response => {
			...
			saveEntity(){// 保存和修改功能
				...
				if(mode == 'NEW'){
					postJson('api/config/chaincode', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}else{
					putJson('api/config/chaincode', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}
			...
			deleteEntity(item){// 删除功能
	            this.$confirm(this.$t('cfgcc.delete_confirm', [item.id]), this.$t('confirm'), {
	              	confirmButtonText: this.$t('actions.ok'),
	              	cancelButtonText: this.$t('actions.cancel'),
	              	type: 'warning'
	            }).then(() => {
	              	deleteJson('api/config/chaincode/' + item.id, {}, response => {
	              		this.$message({type: 'info', message: this.$t('cfgcc.delete_success', [item.id])})
						this.doSearch()
	              	})
	            }).catch(() => {
	              	this.$message({
			...
			getFieldValue(source, keys){
				...
				for(let i=0;i<items.length;i++){
					if(value != null) value = value[items[i]]	
				}
				return value
			}
    	}
