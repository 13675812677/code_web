<template>
  	<div class="app-container">
		...
		<OrganizationEditor ref="editor" @onOK="saveEntity"></OrganizationEditor>
  	</div>
...
<script>
	...
	import OrganizationEditor from '@/views/sysorganization/editor'
  	export default {
		// 注册依赖组件
    	...
    	data() {
        		...
        		header: [
					...
					{title: this.$t('organization.sdkUrl'), field: 'sdkUrl', class: ''},
					{title: this.$t('organization.active'), field: 'active', class: 'center', format: this.formatActive}
        		],
				// 翻页数据格式
				pagination : {
					page: 1,
    	...
    	methods: {
			doSearch() {// 执行查询
				getJson('api/sys/organization/paginate', this.conditions, response => {
			...
			saveEntity(){// 保存和修改功能
				...
				if(mode == 'NEW'){
					postJson('api/sys/organization', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}else{
					putJson('api/sys/organization', entity, response =>{
						this.$refs.editor.close()
						this.doSearch()
					})
				}
			...
			deleteEntity(item){// 删除功能
	            this.$confirm(this.$t('organization.delete_confirm', [item.name]), this.$t('confirm'), {
	              	confirmButtonText: this.$t('actions.ok'),
	              	cancelButtonText: this.$t('actions.cancel'),
	              	type: 'warning'
	            }).then(() => {
	              	deleteJson('api/sys/organization/' + item.code, {}, response => {
	              		this.$message({type: 'info', message: this.$t('organization.delete_success', [item.name])})
						this.doSearch()
	              	})
	            }).catch(() => {
	              	this.$message({
